...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not win:
            logger.info(f"Steam penceresi bulunamadı")
            return False
        ...
        if not colorful.any():
            logger.info(f"Avatar rengi bulunamadı — tahminle tıklanıyor")
...
def _handle_steam_profile_selection() -> bool:
    logger.info(f"Profil seçim dialogu kontrol ediliyor...")
    win = _get_steam_window_rect()
    if not win:
        return False
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            logger.info(f"Profil dialogu yok — Steam zaten giriş yapmış")
            return False
    except ImportError:
        pass
    except Exception:
    ...
    logger.info(f"Profil seçimi tespit edildi — ilk profile tıklanıyor")
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    logger.info(f"Steam başlatılıyor...")
    if is_mac():
    ...
    logger.info(f"Steam başlatılamadı")
    return False
